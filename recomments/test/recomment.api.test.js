...
describe('대댓글 테스트', () => {
    ...
    describe('3. 대댓글 수정', () => {
        test('1. 대댓글이 존재하지 않으면 수정 실패', async () => {
            const data = await request(app)
                .patch(`/recomment/${recommentId}123`)
                .set({ authorization: token })
                .send({ content: '수정입니다' })
            expect(data._body.message).toEqual(
                '해당 대댓글이 존재하지 않습니다'
            )
        })
        test('2. 대댓글 작성자 본인이 아니면, 수정 실패', async () => {
            const data = await request(app)
                .patch(`/recomment/${recommentId}`)
                .set({ authorization: otherToken })
                .send({ content: '다른사람이에요' })
            expect(data._body.message).toEqual(
    ...
    describe('3. 대댓글 삭제', () => {
        test('1. 대댓글이 존재하지 않으면 삭제 실패', async () => {
            const data = await request(app)
                .delete(`/recomment/${recommentId}123`)
                .set({ authorization: token })
                .send({ content: '수정입니다' })
            expect(data._body.message).toEqual(
                '해당 대댓글이 존재하지 않습니다'
